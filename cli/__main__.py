...
def main(
    version: Optional[bool] = Option(
        ...
        is_flag=True,
        callback=lambda v: console.print(
        f"boilerplates version {__version__}") or sys.exit(0) if v else None,
        is_eager=True,
    ),
...
def init_app() -> None:
    ...
    try:
        ...
        logger.debug(
            f"Registering {len(module_classes)} template-based modules")

        for name, module_cls in module_classes:
            try:
                logger.debug(
                    f"Registering module class: {module_cls.__name__}")
        ...
        if failed_registrations:
            logger.info(
                f"Module registration failures: {len(failed_registrations)}")

    except Exception as e:
        error_details = []
        if failed_imports:
            error_details.extend(["Import failures:"] +
                                 [f"  - {err}" for err in failed_imports])
        if failed_registrations:
