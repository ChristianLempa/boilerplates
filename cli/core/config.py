...
class ConfigManager:
    ...
    def _write_config(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Configuration written atomically to {self.config_path}")
            ...
            if tmp_path:
                try:
                    ...
                    logger.warning(
                        f"Failed to clean up temporary file: {tmp_path}")
        ...
        if "defaults" in config:
            for module_name, module_defaults in config["defaults"].items():
                ...
                self._validate_string_length(
                    module_name, "Module name", max_length=100)
        ...
        if "preferences" in config:
            ...
            if "editor" in preferences:
                if not isinstance(preferences["editor"], str):
                    raise ConfigValidationError(
                        "Preference 'editor' must be a string")
                self._validate_string_length(
            ...
            if "output_dir" in preferences:
                ...
                if output_dir is not None:
                    ...
                    self._validate_path_string(
                        output_dir, "Preference 'output_dir'")
            ...
            if "library_paths" in preferences:
                ...
                for i, path in enumerate(preferences["library_paths"]):
                    ...
                    self._validate_path_string(
                        path, f"Library path at index {i}")
            ...
            for i, library in enumerate(libraries):
                ...
                for field in required_fields:
                    if field not in library:
                        raise ConfigValidationError(
                            f"Library at index {i} missing required field '{field}'")
                    ...
                    self._validate_string_length(
                        library[field], f"Library '{field}' at index {i}", max_length=500)
                ...
                if "branch" in library:
                    ...
                    self._validate_string_length(
                        library["branch"], f"Library 'branch' at index {i}", max_length=200)
    ...
    def set_defaults(self, module_name: str, defaults: Dict[str, Any]) -> None:
        ...
        self._validate_list_length(
            list(defaults.keys()), "Defaults dictionary")
        ...
        for var_name, var_value in defaults.items():
            ...
            if isinstance(var_value, str):
                self._validate_string_length(
                    var_value, f"Value for '{var_name}'")
            elif isinstance(var_value, list):
                self._validate_list_length(
                    var_value, f"Value for '{var_name}'")
            elif var_value is not None and not isinstance(var_value, (bool, int, float)):
    ...
    def set_preference(self, key: str, value: Any) -> None:
        ...
        if not isinstance(key, str) or not key:
            raise ConfigValidationError(
                "Preference key must be a non-empty string")
        ...
        if key == "editor":
            if not isinstance(value, str):
                raise ConfigValidationError(
                    "Preference 'editor' must be a string")
            self._validate_string_length(
                value, "Preference 'editor'", max_length=100)
            ...
            if library.get("name") == name:
                ...
                if "url" in kwargs:
                    ...
                    if not isinstance(url, str) or not url:
                        raise ConfigValidationError(
                            "Library URL must be a non-empty string")
                    self._validate_string_length(
                        url, "Library URL", max_length=500)
                    library["url"] = url
                ...
                if "enabled" in kwargs:
                    ...
                    if not isinstance(enabled, bool):
                        raise ConfigValidationError(
                            "Library enabled must be a boolean")
