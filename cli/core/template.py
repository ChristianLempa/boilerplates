...
def _get_common_jinja_suggestions(error_msg: str, available_vars: set) -> List[str]:
    ...
    if 'undefined' in error_lower or 'is not defined' in error_lower:
        ...
        if var_match:
            ...
            similar = [v for v in available_vars if undefined_var.lower(
            ) in v.lower() or v.lower() in undefined_var.lower()]
            if similar:
                suggestions.append(
            ...
            suggestions.append(
                "Or use the Jinja2 default filter: {{ " + undefined_var + " | default('value') }}")
        else:
            suggestions.append(
    ...
    # Filter errors
        ...
        suggestions.append(
            "Verify the filter exists in Jinja2 built-in filters")
        suggestions.append("Make sure filter arguments are properly formatted")

    # Template not found
    elif 'not found' in error_lower or 'does not exist' in error_lower:
        suggestions.append(
            "Check that the included/imported template file exists")
        suggestions.append(
    ...
    if not suggestions:
        suggestions.append(
            "Check the Jinja2 template syntax and variable usage")
        suggestions.append(
...
class TemplateMetadata:
    ...
    def _validate_metadata(template_data: dict) -> None:
        ...
        if metadata_section is None:
            raise ValueError(
                "Template format error: missing 'metadata' section")
...
class Template:
    ...
    def __init__(self, template_dir: Path, library_name: str) -> None:
        ...
        # New attribute
        self.__template_files: Optional[List[TemplateFile]] = None
        ...
        try:
            ...
            with open(main_template_path, "r", encoding="utf-8") as f:
                ...
                if not valid_docs:
                    raise ValueError(
                        "Template file contains no valid YAML data")

                if len(valid_docs) > 1:
                    logger.warning(
                        f"Template file contains multiple YAML documents, using the first one")
            ...
            if not isinstance(self._template_data, dict):
                raise ValueError(
                    "Template file must contain a valid YAML dictionary")
            ...
            logger.info(
                f"Loaded template '{self.id}' (v{self.metadata.version})")
            ...
            logger.error(
                f"File I/O error loading template {template_dir}: {e}")
            raise TemplateLoadError(
    ...
    def _merge_specs(self, module_specs: dict, template_specs: dict) -> dict:
        ...
        module_collection = VariableCollection(
            module_specs) if module_specs else VariableCollection({})
        ...
        for root, _, files in os.walk(self.template_dir):
            for filename in files:
                ...
                template_files.append(TemplateFile(
                    relative_path=relative_path, file_type=file_type, output_path=output_path))
            ...
            if template_file.file_type == 'j2':
                ...
                try:
                    ...
                    syntax_errors.append(
                        f"  - {relative_path}: File I/O error: {e}")
                except Exception as e:
    ...
    def _validate_kind(template_data: dict) -> None:
        ...
        if not template_data.get("kind"):
            raise TemplateValidationError(
                "Template format error: missing 'kind' field")
        ...
        for section in variables.get_sections().values():
            for var_name, variable in section.variables.items():
                if variable.autogenerated and (variable.value is None or variable.value == ""):
                    ...
                    logger.debug(
                        f"Auto-generated value for variable '{var_name}'")

        if debug:
            logger.info(f"Rendering template '{self.id}' in debug mode")
            logger.info(
                f"Available variables: {sorted(variable_values.keys())}")
        ...
        for template_file in self.template_files:  # Iterate over TemplateFile objects
            if template_file.file_type == 'j2':
                try:
                    ...
                    rendered_files[str(template_file.output_path)
                                   ] = rendered_content
                    ...
                    raise TemplateRenderError(
                        ...
                        variable_context={
                            k: str(v) for k, v in variable_values.items()} if debug else {},
                ...
                try:
                    ...
                    with open(file_path, "r", encoding="utf-8") as f:
                        content = f.read()
                        rendered_files[str(
                            template_file.output_path)] = content
                except (IOError, OSError) as e:
    ...
    def variables(self) -> VariableCollection:
        if self.__variables is None:
            ...
            self._validate_variable_definitions(
                self.used_variables, self.merged_specs)
            # Filter specs to only used variables
            filtered_specs = self._filter_specs_to_used(
                self.used_variables, self.merged_specs, self.module_specs, self.template_specs)
