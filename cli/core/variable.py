...
class Variable:
    ...
    def __init__(self, data: dict[str, Any]) -> None:
        ...
        self.description: Optional[str] = data.get(
            "description") or data.get("display", "")
        self.type: str = data.get("type", "str")
        self.options: Optional[List[Any]] = data.get("options", [])
        self.prompt: Optional[str] = data.get("prompt")
        self.value: Any = data.get("value") if data.get(
            "value") is not None else data.get("default")
        ...
        if self.value is not None:
            try:
                ...
                raise ValueError(
                    f"Invalid default for variable '{self.name}': {exc}")
    ...
    def _convert_enum(self, value: Any) -> Optional[str]:
        ...
        if self.options and val not in self.options:
            raise ValueError(
                f"value must be one of: {', '.join(self.options)}")
        return val
    ...
    def _convert_url(self, value: Any) -> str:
        ...
        if not (parsed.scheme and parsed.netloc):
            raise ValueError(
                "value must be a valid URL (include scheme and host)")
        return val
