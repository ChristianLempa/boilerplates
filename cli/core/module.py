    ...
    for var_option in var_options:
        if '=' in var_option:
            ...
            if extra_args:
                ...
                logger.warning(
                    f"No value provided for variable '{var_option}'")
...
class Module(ABC):
    ...
    def list(
        ...
        for template_dir, library_name in entries:
            try:
                ...
                logger.error(
                    f"Failed to load template from {template_dir}: {exc}")
                continue
        ...
        if filtered_templates:
            if raw:
                ...
                for template in filtered_templates:
                    ...
                    version = str(
                        template.metadata.version) if template.metadata.version else "-"
    ...
    def search(
        self,
        query: str = Argument(...,
                              help="Search string to filter templates by ID")
    ) -> list[Template]:
        ...
        filtered_templates = [
            t for t in templates if query.lower() in t.id.lower()]
        ...
        if filtered_templates:
            ...
            self.display.display_warning(
                f"No templates found matching '{query}'", context=f"module '{self.name}'")
    ...
    def show(
        ...
        if template.variables:
            ...
            if config_defaults:
                logger.debug(
                    f"Loading config defaults for module '{self.name}'")
    ...
    def _apply_cli_overrides(self, template: Template, var: Optional[List[str]], ctx=None) -> None:
        ...
        if cli_overrides:
            logger.info(
                f"Received {len(cli_overrides)} variable overrides from CLI")
    ...
    def _execute_dry_run(self, id: str, output_dir: Path, rendered_files: Dict[str, str], show_files: bool) -> None:
        ...
        self.display.display_heading(
            "Directory Operations", icon_type="folder")
    ...
    def generate(
        ...
        interactive: bool = Option(True, "--interactive/--no-interactive",
                                   "-i/-n", help="Enable interactive prompting for variables"),
        var: Optional[list[str]] = Option(
            None, "--var", "-v", help="Variable override (repeatable). Supports: KEY=VALUE or KEY VALUE"),
        dry_run: bool = Option(
            False, "--dry-run", help="Preview template generation without writing files"),
        show_files: bool = Option(
            False, "--show-files", help="Display generated file contents in plain text (use with --dry-run)"),
        ...
        try:
            ...
            if not rendered_files:
                display.display_error(
                    "Template rendering returned no files", context="template generation")
                raise Exit(code=1)
            ...
            if dry_run:
                if not quiet:
                    self._execute_dry_run(
                        id, output_dir, rendered_files, show_files)
            else:
                self._write_generated_files(
                    output_dir, rendered_files, quiet=quiet)
            ...
            display.display_template_render_error(
                e, context=f"template '{id}'")
            raise Exit(code=1)
        except Exception as e:
            display.display_error(
                str(e), context=f"generating template '{id}'")
            raise Exit(code=1)
    ...
    def config_get(
        ...
        if var_name:
            ...
            if value is not None:
                ...
                self.display.display_warning(
                    f"No default set for variable '{var_name}'", context=f"module '{self.name}'")
        else:
    ...
    def config_set(
        self,
        var_name: str = Argument(...,
                                 help="Variable name or var=value format"),
        ...
        if '=' in var_name and value is None:
            ...
            console.print(
                f"[dim]Usage: defaults set VAR_NAME VALUE or defaults set VAR_NAME=VALUE[/dim]")
            raise Exit(code=1)
    ...
    def config_clear(
        ...
        if var_name:
            ...
            if var_name in defaults:
                ...
                config.set_defaults(self.name, defaults)
                self.display.display_success(
                    f"Cleared default for '{var_name}'")
            else:
                self.display.display_error(
                    f"No default found for variable '{var_name}'")
        else:
    ...
    def config_list(self) -> None:
        ...
        console.print(Panel(
            yaml_output, title=f"{self.name.capitalize()} Config", border_style="blue"))
    ...
    def validate(
        ...
        path: Optional[str] = Option(
            None, "--path", "-p", help="Validate a template from a specific directory path"),
        verbose: bool = Option(False, "--verbose", "-v",
                               help="Show detailed validation information"),
        semantic: bool = Option(True, "--semantic/--no-semantic",
        ...
        if template:
            ...
            console.print(
                f"[bold]Validating all {self.name} templates...[/bold]\n")
            ...
            for template_dir, library_name in entries:
                ...
                try:
                    template = Template(
                        template_dir, library_name=library_name)
                    # Trigger validation
                    _ = template.used_variables
                    _ = template.variables
                    valid_count += 1
            ...
            self.display.display_summary_table(
    ...
    def register_cli(cls, app: Typer) -> None:
        ...
        defaults_app = Typer(
            help="Manage default values for template variables")
    ...
    def _load_template_by_id(self, id: str) -> Template:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Template '{id}' could not be loaded: {exc}") from exc
