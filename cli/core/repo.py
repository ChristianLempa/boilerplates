...
def update(
    ...
    if library_name:
        libraries = [lib for lib in libraries if lib.get(
            "name") == library_name]
        if not libraries:
            console_err.print(
    ...
    with Progress(
        ...
        for lib in libraries:
            ...
            if not enabled:
                if verbose:
                    console.print(
                        f"[dim]Skipping disabled library: {name}[/dim]")
...
def add(
    ...
    directory: str = Option("library", "--directory", "-d",
                            help="Directory within repo containing templates (metadata only)"),
    enabled: bool = Option(True, "--enabled/--disabled",
                           help="Enable or disable the library"),
    sync: bool = Option(True, "--sync/--no-sync",
                        help="Sync the library after adding")
) -> None:
        ...
        if not keep_files:
            ...
            if library_path.exists():
                ...
                display.display_success(
                    f"Deleted local files at {library_path}")
            else:
