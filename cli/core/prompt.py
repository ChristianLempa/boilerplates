...
class PromptHandler:
    ...
    def collect_variables(self, variables: VariableCollection) -> dict[str, Any]:
        ...
        for section_key, section in variables.get_sections().items():
            ...
            if not variables.is_section_satisfied(section_key):
                ...
                unsatisfied_keys = [
                    dep for dep in section.needs if not variables.is_section_satisfied(dep)]
                ...
                dep_names = ", ".join(
                    unsatisfied_titles) if unsatisfied_titles else "unknown"
            ...
            self.display.display_section_header(
            ...
            for var_name, variable in section.variables.items():
                ...
                new_value = self._prompt_variable(
                    variable, required=section.required)
        ...
        logger.info(
            f"Variable collection completed. Collected {len(collected)} values")
        return collected
    ...
    def _prompt_variable(self, variable: Variable, required: bool = False) -> Any:
        ...
        while True:
            try:
                ...
                converted = variable.validate_and_convert(
                    raw, check_required=True)
    ...
    def _prompt_bool(self, prompt_text: str, default: Any = None) -> bool | None:
        ...
        converted = default if isinstance(default, bool) else str(
            default).lower() in ("true", "1", "yes", "on")
