...
class Library:
    ...
    def _is_template_draft(self, template_path: Path) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Error checking draft status for {template_path}: {e}")
            return False
    ...
    def find(self, module_name: str, sort_results: bool = False) -> list[tuple[Path, str]]:
        ...
        try:
            for item in module_path.iterdir():
                has_template = item.is_dir() and any((item / f).exists()
                                                     for f in ("template.yaml", "template.yml"))
        ...
        logger.debug(
            f"Found {len(template_dirs)} templates in module '{module_name}'")
...
class LibraryManager:
    ...
    def _load_libraries_from_config(self) -> list[Library]:
        ...
        for i, lib_config in enumerate(library_configs):
            ...
            if not lib_config.get("enabled", True):
                logger.debug(
                    f"Skipping disabled library: {lib_config.get('name')}")
                continue
            ...
            libraries.append(
                Library(name=name, path=library_path, priority=priority))
            logger.debug(
                f"Loaded library '{name}' from {library_path} with priority {priority}")

        if not libraries:
            logger.warning(
                "No libraries loaded. Run 'repo update' to sync libraries.")
    ...
    def find(self, module_name: str, sort_results: bool = False) -> list[tuple[Path, str]]:
        ...
        for library in sorted(self.libraries, key=lambda x: x.priority, reverse=True):
            try:
                templates = library.find(
                    module_name, sort_results=False)  # Sort at the end
