...
class DockerComposeValidator(ContentValidator):
    ...
    def validate(self, content: str, file_path: str) -> ValidationResult:
        ...
        try:
            ...
            if not isinstance(data, dict):
                result.add_error(
                    "Docker Compose file must be a YAML dictionary")
                return result
    ...
    def _validate_service(self, name: str, config: Any, result: ValidationResult) -> None:
        ...
        if not has_image and not has_build:
            result.add_error(
                f"Service '{name}': must specify 'image' or 'build'")
        ...
        if "environment" in config:
            ...
            if isinstance(env, list):
                ...
                keys = [e.split("=")[0]
                        for e in env if isinstance(e, str) and "=" in e]
        ...
        if "ports" in config:
            ...
            if not isinstance(ports, list):
                result.add_warning(
                    f"Service '{name}': 'ports' should be a list")
...
class ValidatorRegistry:
    ...
    def validate_file(self, content: str, file_path: str) -> ValidationResult:
        ...
        if validator:
            logger.debug(
                f"Validating {file_path} with {validator.__class__.__name__}")
