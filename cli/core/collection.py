...
class VariableCollection:
    ...
    def _validate_unique_variable_names(self) -> None:
        ...
        duplicates = {var: sections for var,
                      sections in var_to_sections.items() if len(sections) > 1}
    ...
    def is_section_satisfied(self, section_key: str) -> bool:
        ...
        for dep_key in section.needs:
            ...
            if not dep_section:
                logger.warning(
                    f"Section '{section_key}' depends on missing section '{dep_key}'")
                return False

            # Check if dependency is enabled
            if not dep_section.is_enabled():
                logger.debug(
                    f"Section '{section_key}' dependency '{dep_key}' is disabled")
                return False
    ...
    def _topological_sort(self) -> List[str]:
        ...
        in_degree = {key: len(section.needs)
        ...
        if len(result) != len(self._sections):
            logger.warning(
                "Topological sort incomplete - using original order")
    ...
    def get_satisfied_values(self) -> dict[str, Any]:
        ...
        for section_key, section in self._sections.items():
            ...
            if not section.is_enabled():
                logger.debug(
                    f"Excluding variables from section '{section_key}' - section is disabled")
                continue
    ...
    def apply_defaults(self, defaults: dict[str, Any], origin: str = "cli") -> list[str]:
        ...
        for var_name, value in defaults.items():
            try:
                ...
                if not variable:
                    logger.warning(
                        f"Variable '{var_name}' not found in template")
                    continue
        ...
        if errors:
            logger.warning(
                f"Some defaults failed to apply: {'; '.join(errors)}")
    ...
    def validate_all(self) -> None:
        ...
        for section_key, section in self._sections.items():
            ...
            if not self.is_section_satisfied(section_key) or not section.is_enabled():
                logger.debug(
                    f"Skipping validation for section '{section_key}'")
                continue
            ...
            for var_name, variable in section.variables.items():
                try:
                    ...
                    if variable.type not in ("bool",) and not typed:
                        ...
                        errors.append(
                            f"{msg} (required - cannot be empty)" if variable.is_required() else f"{msg} (empty)")

                except ValueError as e:
                    errors.append(
                        f"{section.key}.{var_name} (invalid format: {e})")
    ...
    def _merge_sections(self, self_section: VariableSection, other_section: VariableSection, origin: str) -> VariableSection:
        ...
        for var_name, other_var in other_section.variables.items():
            if var_name in merged_section.variables:
                ...
                merged_section.variables[var_name] = self_var.clone(
                    update=update)
            else:
